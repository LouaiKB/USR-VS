...
int main(int argc, char* argv[])
{
	...
	while (true)
	{
		...
		for (unsigned int query_number = 0; query_number < num_queries; ++query_number)
		{
			...
			hits_csv << "ZINC ID,USR score,USRCAT score,Tanimoto score,Molecular weight (g/mol),Partition coefficient xlogP,Apolar desolvation (kcal/mol),Polar desolvation (kcal/mol),Hydrogen bond donors,Hydrogen bond acceptors,Polar surface area tPSA (Å^2),Net charge,Rotatable bonds,SMILES,Vendors and annotations\n";
